...
class MaskingSpanDataTest {
	...
	private static final SpanContext SPAN_CONTEXT = SpanContext.create("01234567890123456789012345678901",
			"0123456789012345", TraceFlags.getSampled(), TraceState.getDefault());

	@Test
	void masksStringAttributesAndCachesResults() {
		PiiMasker masker = new PiiMasker();
		SpanData base = TestSpanData.builder()
			...
			.setAttributes(
					Attributes.builder().put("plain", "user@example.com").put(AttributeKey.longKey("n"), 1L).build())
	...
	void masksGenAiEventsAndGenericContentEvents() {
		PiiMasker masker = new PiiMasker();
		EventData promptEvent = EventData.create(TimeUnit.MILLISECONDS.toNanos(1),
				GenAiTelemetrySupport.EVENT_GEN_AI_CONTENT_PROMPT,
				Attributes.of(GenAiTelemetrySupport.GEN_AI_PROMPT, "secret@x.com"));
		EventData completionEvent = EventData.create(TimeUnit.MILLISECONDS.toNanos(2),
				GenAiTelemetrySupport.EVENT_GEN_AI_CONTENT_COMPLETION,
				Attributes.of(GenAiTelemetrySupport.GEN_AI_COMPLETION, "out@y.com"));
		EventData genericContent = EventData.create(TimeUnit.MILLISECONDS.toNanos(3), "gen_ai.content.custom",
				Attributes.of(AttributeKey.stringKey("payload"), "z@z.com"));
		EventData passwordKey = EventData.create(TimeUnit.MILLISECONDS.toNanos(4), "other",
				Attributes.of(AttributeKey.stringKey("user_password"), "secret@evil.com"));
		EventData passthrough = EventData.create(TimeUnit.MILLISECONDS.toNanos(5), "other",
		...
		assertThat(masked.getEvents().get(4).getAttributes().get(AttributeKey.stringKey("safe"))).isEqualTo("visible");
	...
	void masksEventAttributesForPromptAndCompletionSemantics() {
		PiiMasker masker = new PiiMasker();
		EventData promptWrongKey = EventData.create(1L, GenAiTelemetrySupport.EVENT_GEN_AI_CONTENT_PROMPT,
				Attributes.of(AttributeKey.stringKey("other_key"), "a@b.com"));
		EventData completionWrongKey = EventData.create(2L, GenAiTelemetrySupport.EVENT_GEN_AI_CONTENT_COMPLETION,
				Attributes.of(AttributeKey.stringKey("other_key"), "c@d.com"));
		EventData mixedTypes = EventData.create(3L, "evt",
				Attributes.builder()
		...
		assertThat(masked.getEvents().get(2).getAttributes().get(AttributeKey.stringKey("note"))).isEqualTo("visible");
	}

	@Test
	void masksByKeyNameHeuristics() {
		PiiMasker masker = new PiiMasker();
		EventData byPrompt = EventData.create(1L, "custom",
				Attributes.of(AttributeKey.stringKey("meta_prompt_id"), "a@b.com"));
		EventData byCompletion = EventData.create(2L, "custom",
				Attributes.of(AttributeKey.stringKey("completion_meta"), "c@d.com"));
		EventData byPassword = EventData.create(3L, "custom",
	...
	void leavesPlainAttributesWhenEventNameDoesNotMatchGenAiConventions() {
		PiiMasker masker = new PiiMasker();
		EventData emptyName = EventData.create(1L, "",
				Attributes.of(AttributeKey.stringKey("plain_body"), "visible@example.com"));
